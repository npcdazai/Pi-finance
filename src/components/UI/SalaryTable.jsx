...
const FormatYearMonth = ({ isoDate }) => {
  const formatDate = (dateString) => {
    ...
    date.setMonth(date.getMonth() - 1); // Shift date back by 1 month

    return date.toLocaleString("en-US", {
      year: "numeric",
      month: "short",
      timeZone: "Asia/Kolkata", // Specify the timezone here
    });
  };
...
const SalaryTable = () => {
  const [grossSalaryInput, setGrossSalaryInput] = useState(""); // For the new gross salary input
  const [updatedSalaryData, setUpdatedSalaryData] = useState(null); // State to store the updated salary data
  const { getAllData } = useContext(AppContext);

  // Ensure that we have data available either from getAllData or updatedSalaryData
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setUpdatedSalaryData(response.data.data); // Update state with the new data
      }
    } catch (error) {
